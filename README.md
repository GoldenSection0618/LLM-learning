...
| 命名空间 | 里程碑 |
| --- | --- |
| `core` | 1：阶段 0～2；2：阶段 3～5；3：阶段 6～8；4：阶段 9；5：阶段 10；6：可选阶段 11 与阶段 12 |
| `distill` | 1：Teacher 数据闭环；2：序列蒸馏主结论；3：偏好蒸馏完成；4：验证与结项 |
...
- 阶段 12 使用四个 `architecture` 分支推进。选择阶段 11 时先完成对应实验。架构里程碑四完成后，在同一个 `main` 提交上标记 `core-milestone-6`。
