...
| --- | --- |
| 每周投入 | 35 小时 |
| MiniMind 基础与后训练链路 | 预计 4 周 |
| 进入 Qwen 正式后训练 | 第 5 周开始 |
| 完成 Qwen 正式后训练核心项目 | 预计累计 6～7 周 |
| 前期硬件 | RTX 4060 8GB |
| Qwen 正式后训练阶段硬件 | 计划租用 RTX 5090 32GB |

以上时间按每周 35 小时的有效学习与实践投入估算，不包含数据下载、模型训练等无需持续操作的等待时间。进度安排不按原计划进行简单等比例压缩：模型训练、故障排查、实验对照和复盘仍需保留必要时间。

阶段 10 可选旁路的 8～12 小时不计入第 5～7 周核心工期。机动时间不足时直接延后，未完成该旁路不影响阶段 10 验收或第 8 周蒸馏专题启动。阶段 12 按既定路线执行时，该旁路默认跳过；只有希望单独观察领域适应与通用能力回退时再启动。

前两周以训练正确性为主要目标，不以模型生成质量为进度判断标准。第 3～4 周完成 MiniMind 的 SFT、LoRA、DPO、蒸馏和实验规范整理；达到相应验收标准后，从第 5 周开始 Qwen 正式后训练。

### 阶段总览

| 阶段 | 工作内容 | 主要模型 | 预计投入 | 计划窗口 |
| ---: | --- | --- | ---: | --- |
| 0 | 语言模型张量与 loss | 无 | 4～6 小时 | 第 1 周 |
| 1 | 最小训练循环 | Bigram LM | 6～8 小时；拓展另加 2～4 小时 | 第 1 周 |
| 2 | 最小 Transformer | 1M～10M TinyGPT | 14～18 小时 | 第 1 周 |
| 3 | 阅读现代模型结构 | 缩小版 MiniMind | 6～8 小时 | 第 2 周 |
| 4 | 从头预训练 | MiniMind 64M | 16～20 小时 | 第 2 周 |
| 5 | 监督微调 | MiniMind 64M | 16～20 小时 | 第 2～3 周 |
| 6 | LoRA | MiniMind 64M | 6～8 小时 | 第 3 周 |
| 7 | DPO | MiniMind 64M | 14～18 小时 | 第 3 周 |
| 8 | 模型蒸馏 | Qwen3-4B / MiniMind Teacher → MiniMind Student | 10～14 小时 | 第 4 周 |
| 9 | 实验规范整理 | MiniMind | 6～8 小时 | 第 4 周 |
| 10 | 正式后训练 | Qwen3 0.6B / 1.7B / 4B | 50～70 小时 | 第 5～7 周 |
| 10（旁路） | 领域继续预训练（可选） | Qwen3 0.6B Base | 8～12 小时 | 有领域适应专项兴趣时 |
| 11 | 可选 RL 后训练 | Qwen | 按实验确定 | 蒸馏专题完成后 |
| 12 | 架构迁移与受控研究 | GPT-2 Medium / OLMo 3 100M～370M | 205～280 小时 | 第 12～19 周 |
...
| 第 4 周 | 阶段 8～9 | 完成蒸馏实验，使用现成工具固化配置、记录和评估规范 |
| 第 5～6 周 | 阶段 10 主体 | 完成 Qwen 开发模型调试、4B QLoRA SFT 和完整评估 |
| 第 7 周 | 阶段 10 收尾 | 完成公开偏好集上的最小 DPO 流程、通用回归检查、复现检查和蒸馏专题准备 |
| 第 8～11 周 | 蒸馏专题 | 执行独立的 Qwen 蒸馏学习计划；完成后可插入阶段 11，再进入阶段 12 |
| 第 12～19 周 | 阶段 12 | 执行 GPT-2 checkpoint 迁移与 OLMo 3 GQA 受控研究；如先进行可选 RL，窗口顺延 |
...
> **预计投入**　2～4 小时；本练习在阶段 1 验收后执行，不影响进入阶段 2。
...
> **预计投入**　50～70 小时（约 2 周，另保留第 7 周机动时间）
>
